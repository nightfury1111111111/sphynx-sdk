...
export class Pair {
  ...
  public getOutputAmount(inputAmount: TokenAmount): [TokenAmount, Pair] {
    ...
    if (this.routerType && this.routerType === RouterType.pancake) {
      return [
        outputAmount,
        new Pair(RouterType.pancake, inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))
      ]
    }
    return [
      outputAmount,
      new Pair(RouterType.sphynx, inputReserve.add(inputAmount), outputReserve.subtract(outputAmount))
    ]
  }
    ...
    if (!feeOn) {
      ...
      if (!JSBI.equal(kLastParsed, ZERO)) {
        ...
        if (JSBI.greaterThan(rootK, rootKLast)) {
          ...
          const denominator = JSBI.add(JSBI.multiply(rootK, THREE), rootKLast) // FIX: incorrect value here. UPDATE: Aready fixed!
